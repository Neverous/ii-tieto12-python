    ...
    def dataReceived(self, data):
        ...
        while True:
            ...
            if isinstance(obj, PlayerAction):
                self.factory.stats['messages-received'] += 1
                ...
                self.factory.stats['messages-sent'] += 1
                continue
...
class BomberFactory(Factory):
    ...
    def _newGame(self):
        '''Starts new game. Sets all necessary data.'''
        ...
        self.stats['state'] = 'Lobby'

    def appendClient(self, client):
        '''Appends new client and when there is enough people trigger the game start.'''

        if self.state != STATE_WAITING_FOR_PLAYERS:
            return False

        self._clients.append(client)
        self.stats['connected-clients'] += 1
    ...
    def removeClient(self, client):
        '''Removes client and checks if game is playing and only one player is left then triggers game end.'''
    ...
    def checkAlive(self, _ask = None):
        '''Check alive players and trigger game end if only one left or someone asked for it.'''
    ...
    def pregame(self, start = False):
        ...
        self.stats['state'] = 'In-Game'

    def postgame(self):
        '''Ending the game and disconnecting clients.'''
